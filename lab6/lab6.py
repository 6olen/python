...
def missing_digits(n):
    ...
    n_str = str(n) 
    min_d, max_d = n_str[0], n_str[-1] 
    missing = list(set(all_digits[int(min_d):int(max_d) + 1]) - set(n_str)) 
    return len(missing)
...
def count_coins(total):
    ...
    def count_helper(total, coin_index): 
        if total == 0: 
            return 1 
        elif total < 0 or coin_index >= len(coins): 
            return 0 
        else: 
            return count_helper(total - coins[coin_index], coin_index) + count_helper(total, coin_index + 1) 
     
    return count_helper(total, 0)


from operator import sub, mul

def make_anonymous_factorial(n):
